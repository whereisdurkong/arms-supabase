...
export default function ViewAsset() {
    ...
    useEffect(() => {
        const fetchLocationOptions = async () => {
            try {
                ...
                const res = await supabase.from('option_master').select('*');
                const data = res.data || [];
                setMasterData(data);
                const allLocations = [];
                data.forEach(item => {
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                ...
                const res = await supabase.from('assets_master').select('*').eq('asset_id', asset_id).single()
                const data = res.data || [];
                const fetchedData = {
                ...
                if(data.has_components === '1'){
                    const res = await supabase.from('assets_component_master').select('*').eq('asset_id', asset_id);
                    const data  = res.data || [];
    ...
    const handleEditComponent = (component) => {
        ...
        setComponentFormData({
            asset_component_name: component.asset_component_name,
            asset_component_type: component.asset_component_type
        });
        setShowComponentModal(true);
    };

    const handleSaveComponent = async () => {
        if (!componentFormData.asset_component_name.trim()) {
            showAlertMessage('error', 'Validation Error', 'Component name is required');
            return;
        }
        if (!componentFormData.asset_component_type.trim()) {
        ...
        try {
            ...
            if (editingComponent) {
                ...
                // });
                await supabase.from('assets_component_master').update({
                    asset_component_name: componentFormData.asset_component_name,
                    asset_component_type: componentFormData.asset_component_type,
                    updated_by: empInfo.user_name,
                    updated_at: new Date()
                }).eq('asset_component_id', editingComponent.asset_component_id,)


                showAlertMessage('success', 'Success', 'Component updated successfully');
                setTimeout(() => window.location.reload(), 200);
            } else {
                const {data1} = await supabase.from('assets_component_master').select('*');
                const comp_id_length = data1 ? data1.length : 0 ;
                const compo_id = comp_id_length + 1;
                ...
                // });
                await supabase.from('assets_component_master').insert({
                    asset_component_id: compo_id,
                    asset_id: asset_id,
                    asset_component_name: componentFormData.asset_component_name,
                    asset_component_type: componentFormData.asset_component_type,
                    created_by: empInfo.user_name,
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            await supabase.from('assets_master').update({
                asset_name: formData.assetName,
                asset_type: formData.assetType,
                asset_location: formData.location,
                asset_category: formData.category,
                ...
                <Modal.Body style={{ padding: '20px', }}>
                    <Form>
                        <Form.Group className="mb-3">
                            ...
                            <Form.Control
                                type="text"
                                name="asset_component_name"
                                value={componentFormData.asset_component_name}
                        ...
                        <Form.Group className="mb-3">
                            ...
                            <Form.Select
                                name="asset_component_type"
                                value={componentFormData.asset_component_type}
                                onChange={handleComponentInputChange}
                                style={{
                                    border: '2px solid #E2E8F0',
                                    borderRadius: '10px',
                                    ...
                                    <tbody>
                                        {components.map((component, idx) => (
                                            <tr key={component.component_id || idx} style={{ borderBottom: '1px solid #e2e8f0', background: idx % 2 === 0 ? 'white' : '#fafcff' }}>
                                                <td style={{ padding: '10px 16px', fontWeight: '500', color: '#0f172a' }}>{component.asset_component_name}</td>
                                                <td style={{ padding: '10px 16px', color: '#475569' }}>{formatDisplayName(component.asset_component_type)}</td>
