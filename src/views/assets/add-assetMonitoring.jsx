...
export default function AddMonitoringLog() {
    ...
    useEffect(() => {
        const fetchLatestMonitoring = async () => {
            try {
                ...
                const getAll = await supabase.from('monitoring_master').select('*');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const {data} = await supabase.from('monitoring_master').select('*');
            const monitoring_id_length = data.length;
            const monitoring_id = monitoring_id_length + 1;
            ...
            await supabase.from('monitoring_master').insert({
