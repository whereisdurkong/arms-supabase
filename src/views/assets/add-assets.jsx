...
export default function AddAsset() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const {data} = await supabase.from('assets_master').select('*');
        const assets_id_length = data.length;
        const asset_id = assets_id_length + 1;

         const {data1} = await supabase.from('assets_component_master').select('*');
        const comp_id_length = data1.length;
        ...
        console.log(data1)
            ...
            // });
              console.log({
                ...
                await supabase.from('assets_master').insert({
                    asset_id: asset_id,
                    asset_name: formData.assetName,
                    asset_type: formData.assetType,
                    asset_location: formData.location,
                    asset_category: formData.category,
                    trivector: formData.trivector,
                    date_commisioning: new Date(formData.commissioningDate),
                    asset_notes: formData.notes,
                    created_by: empInfo.user_name,
                    created_at: new Date(),
                    has_components: formData.hasComponents || (components && components.length > 0 ? '1' : '0'),
                ...
                await supabase.from('assets_component_master').insert(componentRecords);
                }
