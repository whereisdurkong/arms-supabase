...
export default function ViewSubmittedAsset() {
    ...
    const [showAppropriateInfoModal, setShowAppropriateInfoModal] = useState(false);

    // Permission state variables
    const [canAccessL2Actions, setCanAccessL2Actions] = useState(false);
    const [canAccessL3Actions, setCanAccessL3Actions] = useState(false);
    ...
    useEffect(() => {
        const fetchData = async () => {
            setIsLoading(true);
            console.log('!!!!!!!!!!!!!!!!!!!!!!!!!! \nUSER POSITION: ', empInfo.emp_position)
            try {
                // const res = await axios.get(`${config.baseApi}/assetsAnalysis/get-submitted-assets-by-id`, {
                //     params: { id: asset_analysis_id }
                // });
                const res = await supabase.from('asset_analysis_master').select('*').eq('asset_analysis_id', asset_analysis_id).single();
                const data = res.data || {};
                ...
                const assetres = await supabase.from('assets_master').select('*').eq('asset_id', data.asset_id).single();
                ...
                // Calculate permissions directly from the fetched data
                const hasL2Access = data.level1 === '1' && data.level2 !== '1' && data.level3 !== '1';
                const hasL3Access = data.level1 === '1' && data.level2 === '1' && data.level3 !== '1';

                setCanAccessL2Actions(hasL2Access);
                setCanAccessL3Actions(hasL3Access);
                ...
                if (data.criticality_analysis_status) {
                    ...
                    // Show modals immediately based on calculated permissions
                    if (hasL2Access && empInfo.emp_position !== 'l1') {
                ...
                const componentRes = await supabase.from('assets_component_master').select('*').eq('asset_component_id', data.asset_component_id).single();
                ...
                const res1 = await supabase.from('asset_analysis_master').select('*')
        ...
        fetchData();
    }, [asset_analysis_id, showAlertMessage, empInfo.emp_position]);
    ...
    const handleCriticalitySave = async (selectedStatus) => {
        ...
        if (originalCriticalityStatus === 'Severe' && selectedStatus !== 'Severe') {
            try {
                ...
                const response = await supabase.from('asset_analysis_master').update({
                     action_taken: null,
                    documentation: null,
                    updated_by: empInfo.username,
                    updated_at: new Date()
        ...
        try {
            ...
            if (selectedStatus === "Verify/Abnormal" && !resamplingSchedule && canAccessL2Actions) {
                setShowSamplingModal(true);
            }
        } catch (err) {
    ...
    const SevereActionModal = React.memo(() => {
        ...
        // Pre-populate existing actions when modal opens for editing
        useEffect(() => {
            if (showSevereModal && data.action_taken) {
                setSevereSelectedAction(data.action_taken);
            }
        }, [showSevereModal, data.action_taken]);
    ...
    const [continuePermission, setContinuePermission] = useState(false);
    const [resamplingSchedulePermission, setResamplingSchedulePermission] = useState(null);
    const [criticalityEditPermission, setCriticalityEditPermission] = useState(null);

    const [showAppropriateModal, setShowAppropriateModal] = useState(false);

    // Modal to display appropriate action info (bell/note button)
    const AppropriateInfoModal = () => {
        if (!data.appropriate_action) return null;

        const isAppropriate = data.appropriate_action === 'yes';

        // Dynamic colors based on appropriate_action status
        const theme = isAppropriate ? {
            borderColor: '#10b981',
            headerGradient: 'linear-gradient(135deg, #10b981 0%, #059669 100%)',
            headerBorderColor: '#10b981',
            headerIcon: 'check-circle',
            headerTitle: 'Actions Taken Appropriately',
            statusBgColor: '#f0fdf4',
            statusBorderColor: '#10b981',
            statusIcon: 'check-circle',
            statusIconColor: '#10b981',
            statusTextColor: '#065f46',
            statusSubtextColor: '#065f46',
            statusMessage: isAppropriate
                ? '✓ The recommended actions were taken appropriately as per Level 3 review.'
                : '✗ The recommended actions were not taken appropriately as per Level 3 review.',
            notesBgColor: '#f0fdf4',
            notesBorderColor: '#bbf7d0',
            notesTextColor: '#065f46'
        } : {
            borderColor: '#dc2626',
            headerGradient: 'linear-gradient(135deg, #dc2626 0%, #ef4444 100%)',
            headerBorderColor: '#dc2626',
            headerIcon: 'bell',
            headerTitle: 'Action Not Taken Appropriately',
            statusBgColor: '#fef2f2',
            statusBorderColor: '#dc2626',
            statusIcon: 'alert-triangle',
            statusIconColor: '#dc2626',
            statusTextColor: '#991b1b',
            statusSubtextColor: '#7f1d1d',
            statusMessage: isAppropriate
                ? 'The recommended actions were taken appropriately as per Level 3 review.'
                : 'The recommended actions were not taken appropriately as per Level 3 review.',
            notesBgColor: '#fffbeb',
            notesBorderColor: '#fde68a',
            notesTextColor: '#78350f'
        };

        return (
            <Modal
                show={showAppropriateInfoModal}
                onHide={() => setShowAppropriateInfoModal(false)}
                centered
                size="md"
                contentClassName="border-0 bg-transparent"
            >
                <div style={{
                    ...
                    <Modal.Header
                        ...
                        style={{
                            ...
                            borderBottom: `2px solid ${theme.headerBorderColor}`,
                            borderTopLeftRadius: '20px',
                            borderTopRightRadius: '20px'
                        }}
                    >
                        <Modal.Title style={{ color: 'white', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '10px' }}>
                            <FeatherIcon icon={theme.headerIcon} size={20} />
                            <span>{theme.headerTitle}</span>
                        </Modal.Title>
                    </Modal.Header>

                    <Modal.Body style={{ padding: '24px' }}>
                        {/* Appropriate Action Status */}
                        <div style={{
                            marginBottom: '24px',
                            padding: '16px',
                            background: theme.statusBgColor,
                            borderRadius: '12px',
                            borderLeft: `4px solid ${theme.statusBorderColor}`
                        }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
                                <FeatherIcon icon={theme.statusIcon} size={20} color={theme.statusIconColor} />
                                <span style={{ fontWeight: '700', color: theme.statusTextColor, fontSize: '16px' }}>
                                    Appropriate Action Status: {isAppropriate ? '✓ Taken' : '✗ Not Taken'}
                                </span>
                            </div>
                            <p style={{ margin: 0, color: theme.statusSubtextColor, fontSize: '13px', marginLeft: '32px' }}>
                                {theme.statusMessage}
                            </p>
                        </div>
                        ...
                        <div style={{ marginBottom: '24px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                                <FeatherIcon icon="edit-3" size={18} color={isAppropriate ? '#10b981' : '#f59e0b'} />
                                <span style={{ fontWeight: '600', color: '#2d1f0f', fontSize: '15px' }}>Action Notes</span>
                            </div>
                            <div style={{
                                background: theme.notesBgColor,
                                padding: '16px',
                                borderRadius: '12px',
                                border: `1px solid ${theme.notesBorderColor}`,
                                color: theme.notesTextColor,
                                fontSize: '14px',
                                lineHeight: '1.6',
                                whiteSpace: 'pre-wrap',
                                wordBreak: 'break-word'
                            }}>
                                {data.action_notes || 'No notes provided'}
                            </div>
                        </div>
                        ...
                        <div style={{
                            ...
                            <div>
                                <div style={{ fontSize: '12px', color: '#8b7355', marginBottom: '4px' }}>
                                    <FeatherIcon icon="user" size={12} style={{ marginRight: '4px' }} />
                                    Posted By (Level 3 Personnel)
                                </div>
                                <div style={{ fontWeight: '600', color: '#2d1f0f' }}>
                    ...
                    <Modal.Footer style={{
                        ...
                        <button
                            onClick={() => setShowAppropriateInfoModal(false)}
                            style={{
                                background: isAppropriate
    ...
    // Modal for "Yes/No" response - Appropriate Actions Taken
    const AppropriateActionModal = () => {
        const [notes, setNotes] = useState('');
        const [isSubmitting, setIsSubmitting] = useState(false);
        const [actionTaken, setActionTaken] = useState(''); // New state for dropdown

        const handleSubmit = async () => {
            if (!actionTaken) {
                showAlertMessage('error', 'Selection Required', 'Please select whether appropriate actions were taken or not.');
                return;
            }

            if (actionTaken === 'no' && !notes.trim()) {
                showAlertMessage('error', 'Notes Required', 'Please provide notes about the actions taken.');
                return;
            }
            ...
            try {
                ...
                if (actionTaken === 'no') {
                    // await axios.post(`${config.baseApi}/assetsAnalysis/update-level-one`, {
                    //     asset_analysis_id: asset_analysis_id,
                    //     updated_by: empInfo.user_name,
                    //     appropriate_action: actionTaken,
                    //     action_notes: notes
                ...
                console.error('Unable to save action notes:', err);
                showAlertMessage('error', 'Error', 'Failed to save your response. Please try again.');
            } finally {
        ...
        return (
            <Modal
                ...
                contentClassName="border-0 bg-transparent"
            >
                <div style={{ borderRadius: '20px', overflow: 'hidden', border: '2px solid #ffa600' }}>
                    <Modal.Header
                        closeButton
                        style={{
                            background: 'linear-gradient(45deg, #EAB56F, #F9982F, #E37239)',
                            borderBottom: `2px solid #E37239`,
                            borderTopLeftRadius: '20px',
                            borderTopRightRadius: '20px',
                            transition: 'all 0.3s ease'
                        }}
                    >
                        <Modal.Title style={{ color: 'white', fontWeight: 'bold' }}>
                            <FeatherIcon icon="clipboard" size={18} style={{ marginRight: '8px' }} />
                        ...
                        <div style={{ marginBottom: '20px' }}>
                            ...
                            <div style={{ marginBottom: '24px' }}>
                                <label style={{ fontSize: '14px', fontWeight: '600', color: COLORS.dark, marginBottom: '8px', display: 'block' }}>
                                    <FeatherIcon icon="check-circle" size={14} color={'#059669'} style={{ marginRight: '6px', verticalAlign: 'middle' }} />
                                    Were appropriate actions taken? <span style={{ color: COLORS.danger }}>*</span>
                                </label>
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setNotes(''); // Clear notes when selection changes
                                    }}
                                    style={{
                                        width: '100%',
                                        padding: '12px',
                                        borderRadius: '10px',
                                        border: `2px solid ${actionTaken ? (actionTaken === 'yes' ? COLORS.success : COLORS.danger) : COLORS.primary}`,
                                        fontSize: '14px',
                                        backgroundColor: 'white',
                                        cursor: 'pointer',
                                        outline: 'none',
                                ...
                                >
                                    ...
                                    <option value="yes" style={{ color: '#10b981' }}>✓ Yes - Appropriate actions were taken</option>
                                    <option value="no" style={{ color: '#ef4444' }}>✗ No - Actions were not taken appropriately</option>
                                </select>
                                ...
                                {actionTaken === 'yes' && (
                                    <div style={{
                                        ...
                                        <FeatherIcon icon="info" size={14} color={COLORS.success} />
                                        <span style={{ fontSize: '12px', color: '#166534' }}>
                                            You can provide notes below
                                        </span>
                                    </div>
                                )}
                                ...
                                {actionTaken === 'no' && (
                                    <div style={{
                                        ...
                                        <FeatherIcon icon="alert-triangle" size={14} color={COLORS.danger} />
                                        <span style={{ fontSize: '12px', color: '#991b1b' }}>
                                            Actions were not taken appropriate. You may provide notes below.
                                        </span>
                                    </div>
                                )}
                            </div>

                            {/* Notes Section - Only show if Yes is selected OR show optional for No */}
                            {(actionTaken === 'yes' || actionTaken === 'no') && (
                                <>
                                    <label style={{ fontSize: '14px', fontWeight: '600', color: COLORS.dark, marginBottom: '8px', display: 'block' }}>
                                        <FeatherIcon icon="edit-3" size={14} color={'#059669'} style={{ marginRight: '6px', verticalAlign: 'middle' }} />
                                        Action Notes {actionTaken === 'no' && <span style={{ color: COLORS.danger }}>*</span>}
                                        {actionTaken === 'yes' && <span style={{ fontSize: '12px', fontWeight: 'normal', color: COLORS.gray }}> (Optional)</span>}
                                    </label>
                                    ...
                                    <Form.Control
                                        ...
                                        placeholder={actionTaken === 'yes'
                                            ? "Optional: Provide any additional information, etc..."
                                            : " Provide reason actions were not taken appropriately, or any additional context..."}
                                        style={{
                                            borderRadius: '10px',
                                            border: `2px solid ${actionTaken === 'yes' ? COLORS.success : COLORS.danger}`,
                                            fontSize: '14px',
                                            padding: '12px',
                                            resize: 'vertical'
                                        }}
                                        autoFocus={actionTaken === 'yes'}
                                    />

                                    <div style={{ fontSize: '12px', color: COLORS.gray, marginTop: '8px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                                        <FeatherIcon icon="info" size={12} />
                                        <span>This documentation will be saved to the asset analysis record.</span>
                                    </div>
                                </>
                            )}
                        </div>
                    ...
                    <Modal.Footer style={{ borderTop: `1px solid ${COLORS.primary}40`, padding: '16px 24px', background: 'white', borderBottomLeftRadius: '20px', borderBottomRightRadius: '20px' }}>
                        ...
                        <button
                            ...
                            disabled={isSubmitting || !actionTaken}
                            style={{
                                ...
                                gap: '8px',
                                opacity: isSubmitting ? 0.6 : 1,
                                cursor: (!actionTaken || isSubmitting) ? 'not-allowed' : 'pointer'
                            }}
                        >
                            {isSubmitting ? (
                                <><Spinner animation="border" size="sm" /> Submitting...</>
                            ) : (
                                <><FeatherIcon icon="save" size={18} /> Submit</>
                            )}
                        </button>
                    </Modal.Footer>
                </div>
    ...
    return (
        <div style={{ background: 'radial-gradient(circle at 10% 30%, #254252 0%, #171C2D 100%)', minHeight: '100vh', padding: 'clamp(20px, 5vw, 40px)', position: 'relative', overflow: 'hidden', paddingTop: 'clamp(30px, 6vw, 50px)' }}>
            ...
            {/* L1 users cannot see any modals - hide all action modals for L1 */}
            {empInfo.emp_position !== 'l1' && empInfo.emp_position !== 'l3' && canAccessL2Actions && (
                <>
            ...
            {/* L3 users can see appropriate modal only */}
            {empInfo.emp_position === 'l3' && canAccessL3Actions && <AppropriateActionModal />}
                ...
                <Container fluid style={{ paddingBottom: '60px', paddingLeft: 'clamp(8px, 3vw, 15px)', paddingRight: 'clamp(8px, 3vw, 15px)' }}>
                    <div style={{ marginBottom: '32px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px', marginBottom: '16px' }}>
                            ...
                            {/* Bell/Note Button - Show when level1='1', level2='1', level3='1', and appropriate_action='yes' */}
                            {data.level1 === '1' && data.level2 === '1' && data.level3 === '1' && data.appropriate_action === 'yes' && (
                                <button
                                    onClick={() => setShowAppropriateInfoModal(true)}
                                    style={{
                                        background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', // Changed to green
                                        border: 'none',
                                        borderRadius: '50px',
                                        padding: '10px 20px',
                                        color: 'white',
                            ...
                            {/* Bell/Note Button - Show when appropriate_action and action_notes have values */}
                            {data.appropriate_action && data.action_notes && (
                                <button
                                    onClick={() => setShowAppropriateInfoModal(true)}
                                    style={{
                            ...
                            {/* Continue button - Only L2 users with access can see */}
                            {continueButton && empInfo.emp_position === 'l2' && canAccessL2Actions && (
                                <button
                            ...
                            {/* L3 appropriate actions section - Only L3 users with access can see */}
                            {empInfo.emp_position === 'l3' && canAccessL3Actions && (
                                <div style={{
                                    display: 'flex',
                                    alignItems: 'center',
                                    justifyContent: 'space-between',
                                    ...
                                    <div
                                        onClick={() => { setShowAppropriateModal(true) }}
                                        style={{
                                            width: '50px',
                                            height: '50px',
                                            background: 'linear-gradient(135deg, #f59e0b 0%, #f97316 100%)',
                                        ...
                                        onMouseEnter={(e) => {
                                            e.currentTarget.style.transform = 'scale(1.05)';
                                            e.currentTarget.style.boxShadow = '0 15px 35px -10px #e39f39';
                                        }}
                                        onMouseLeave={(e) => {
                                            e.currentTarget.style.transform = 'scale(1)';
                                            e.currentTarget.style.boxShadow = '0 10px 30px -10px #e3a739';
                                        }}
                                    >
                                ...
                                <div style={{
                                    ...
                                    <div style={{
                                        ...
                                        <div style={{
                                            ...
                                            ) : criticalityStatus === 'Verify/Abnormal' && canAccessL2Actions && empInfo.emp_position === 'l2' ? (
                                    ...
                                    <div style={{
                                        ...
                                        <div style={{
                                            ...
                                            {/* Edit Criticality button - Only L2 users with access */}
                                            {canAccessL2Actions && empInfo.emp_position === 'l2' && (
                                                <button
                    ...
                    {data.criticality_analysis_status === 'Severe' && (
                        <>
                            <div
                                className="modern-card"
                                style={{
                            ...
                            >
                                ...
                                <div style={{ padding: '24px', position: 'relative', zIndex: 1 }}>
                                    {/* Header Section - Added Edit Button for Severe criticality */}
                                    <div style={{
                                        display: 'flex',
                                        alignItems: 'center',
                                        justifyContent: 'space-between',
                                        ...
                                        {/* Edit Button for L2 users with Severe criticality */}
                                        {canAccessL2Actions && empInfo.emp_position === 'l2' && criticalityStatus === 'Severe' && (
                                            <button
                                                onClick={() => setShowSevereModal(true)}
                                                style={{
                                                    background: 'linear-gradient(135deg, #f0f0f0 0%, #e0e0e0 100%)',
                                                    border: 'none',
                                                    borderRadius: '10px',
                                    ...
                                    <div style={{
                                        ...
                                        {data.action_taken && data.action_taken.trim() !== '' ? (
                                            ...
                                            <div style={{
                                                ...
                                                <FeatherIcon icon="alert-circle" size={24} color={COLORS.danger} style={{ marginRight: '12px' }} />
