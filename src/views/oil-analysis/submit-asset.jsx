...
export default function SubmitAsset() {
    ...
    useEffect(() => {
        const fetchActiveAssets = async () => {
            try {
                ...
                const res = await supabase.from('assets_master').select('*');
                const data = res.data || [];
                const active = data.filter((a) => a.is_active === '1');
                setAssets(active);

                // const res1 = await axios.get(`${config.baseApi}/assets/get-all-components`);
                const res1 = await supabase.from('assets_component_master').select('*');
    ...
    try {
        ...
        // First, get the count for generating asset_analysis_id
        const { count, error: countError } = await supabase
            .from('asset_analysis_master')
            .select('*', { count: 'exact', head: true });
        
        if (countError) throw countError;
        
        const asset_analysis_id = (count || 0) + 1;

        // Parse the oil results
        let parsedResults = oilResults;

        // Prepare the insert data with all columns
        const insertData = {
            asset_id: formData.assetId,
            asset_component_id: formData.componentId,
            asset_analysis_id: asset_analysis_id,
            asset_running_hours: cleanNumericString(formData.runningHours) || null,
            oil_running_hours: cleanNumericString(formData.oilRunningHours) || null,
            recommendations: formData.recommendation || null,
            analysis_date: formData.analysisDate,
            created_by: username,
            ...
            // Wear Metals
            iron: parsedResults.iron || null,
            chrome: parsedResults.chrome || null,
            nickel: parsedResults.nickel || null,
            aluminium: parsedResults.aluminum || parsedResults.aluminium || null,
            lead: parsedResults.lead || null,
            copper: parsedResults.copper || null,
            tin: parsedResults.tin || null,
            titanium: parsedResults.titanium || null,
            silver: parsedResults.silver || null,
            antimony: parsedResults.antimony || null,
            cadmium: parsedResults.cadmium || null,
            manganese: parsedResults.manganese || null,

            // Particle & Wear Indicators
            fatigue_gt_20um: parsedResults.fatigue20 || null,
            non_metallic_gt_20um: parsedResults.nonMetallic20 || null,
            large_fe: parsedResults.largeFe || null,
            fe_wear_severity_index: parsedResults.feWearSeverity || null,
            total_fe_lt_100um: parsedResults.totalFe100 || null,
            cutting_gt_20um: parsedResults.cutting20 || null,
            sliding_gt_20um: parsedResults.sliding20 || null,
            large_fe_percent: parsedResults.largeFePercent || null,
            ...
            cnts_gt4: parsedResults.cnts4 || null,
            cnts_gt6: parsedResults.cnts6 || null,
            cnts_gt14: parsedResults.cnts14 || null,

            // Particle Counts
            particles_5_15um: parsedResults.particles5_15 || null,
            particles_15_25um: parsedResults.particles15_25 || null,
            particles_25_50um: parsedResults.particles25_50 || null,
            particles_50_100um: parsedResults.particles50_100 || null,
            particles_gt100um: parsedResults.particles100 || null,

            // Additives
            molybdenum: parsedResults.molybdenum || null,
            calcium: parsedResults.calcium || null,
            magnesium: parsedResults.magnesium || null,
            phosphorus: parsedResults.phosphorus || null,
            zinc: parsedResults.zinc || null,
            barium: parsedResults.barium || null,
            boron: parsedResults.boron || null,

            // Contaminants
            sodium: parsedResults.sodium || null,
            vanadium: parsedResults.vanadium || null,
            potassium: parsedResults.potassium || null,
            lithium: parsedResults.lithium || null,
            silicon: parsedResults.silicon || null,

            // Fluid Properties
            total_water: parsedResults.totalWater || null,
            bubbles: parsedResults.bubbles || null,
            water: parsedResults.waterContent || parsedResults.water || null,
            glycol_percent: parsedResults.glycol || null,
            soot_percent: parsedResults.sootPercent || null,
            biodiesel_fuel_dilution: parsedResults.biodieselFuelDilution || null,

            // Chemical Properties
            tan: parsedResults.tan || null,
            tbn: parsedResults.tbn || null,
            oxidation: parsedResults.oxidation || null,
            nitration: parsedResults.nitration || null,
            sulfation: parsedResults.sulfation || null,
            ...
            // Integrity
            fluid_integrity: parsedResults.fluidIntegrity || null,
            antiwear_percent: parsedResults.antiwear || null
        };
        ...
        // Insert into asset_analysis_master
        ...
        showAlertMessage('error', 'Error', `Unable to save! ${err.message || 'Please try again.'}`);
    } finally {
        setIsLoading(false);
    }
};
