...
const { data, error } = await supabase
    ...
    const checkForDuplicates = async () => {
        try {
            ...
            const res = await supabase.from('user_master').select('*');
            const data = res.data || [];
...
const handleSaveChanges = async () => {
    ...
    try {
        // Prepare update data - REMOVE id_master from the update object!
        const updateData = {
            ...
            updated_at: new Date().toISOString() // Add timestamp if your table has it
        };

        console.log('Updating user with ID:', id_master);
        console.log('Update data:', updateData);

        // Perform the update - use the correct column name from your database
        const { data, error } = await supabase
            .from('users_master')
            .update(updateData)
            .eq('user_id', id_master) // Assuming the column is 'user_id'
            .select(); // This returns the updated record

        // Check for Supabase errors
        if (error) {
            console.error('Supabase update error:', error);
            showAlertMessage('error', 'Update Failed', error.message || 'Failed to update user profile. Please try again.');
            setIsSaving(false);
            return;
        }
        ...
        if (data && data.length === 0) {
            showAlertMessage('error', 'Update Failed', 'User not found or no changes were made.');
            setIsSaving(false);
            return;
        }

        // Update local user data with the response (or form data)
        ...
        // fetchUserData(); // You'll need to define this outside useEffect or call it
        
    } catch (err) {
        console.error('Unable to update user: ', err);
        showAlertMessage('error', 'Update Failed', err.message || 'Failed to update user profile. Please try again.');
    } finally {
        setIsSaving(false);
    }
};
