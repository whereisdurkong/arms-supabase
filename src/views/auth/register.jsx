...
export default function SignUp1() {
  ...
  const Checker = async () => {
    try {
      const res = await supabase.from('users_master').select('*')
      const data = res.data || [];
  ...
  const Save = async (e) => {
    try {
      ...
      try {
        ...
        const {data} = await supabase.from('users_master').select('*');
        const user_id_length = data.length;
        const user_id = user_id_length + 1;
   
        await supabase.from('users_master').insert({
          user_id: user_id,
